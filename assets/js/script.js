...
function fadeInFirstSlide(captionEl) {
  ...
  if (captionEl && slideshowState.slides[0]) {
    ...
    setTimeout(() => {
      captionEl.style.transition = 'opacity 3s ease-in-out';
...
/**
 * Recursive helper to create DOM elements from JSON structure.
 * Handles 'tag', 'attributes', 'text', 'paragraphs' (as list of p), and nested 'children'/'elements'.
 */
function createElementFromJSON(node) {
  ...
  // Handle "paragraphs" array (Legacy support for Biography style)
  if (node.paragraphs && Array.isArray(node.paragraphs)) {
    // Determine where to append paragraphs. If we are creating a generic wrapper, 
    // we might want an inner container, but here we append directly to 'el'.
    const pContainer = node.tag === 'section' ? document.createElement('div') : el;
    if (node.tag === 'section') {
        pContainer.style.maxWidth = '800px';
        pContainer.style.margin = '0 auto';
        pContainer.style.padding = '2rem';
        el.appendChild(pContainer);
    }

    node.paragraphs.forEach((txt) => {
      const p = document.createElement('p');
      p.innerHTML = txt;
  ...
  // Handle recursive children (supports both 'children' and 'elements' keys)
  const kids = node.children || node.elements;
  if (kids && Array.isArray(kids)) {
    kids.forEach((childNode) => {
      el.appendChild(createElementFromJSON(childNode));
    });
  }
...
function renderContentSection(targetContainer, sectionData) {
  // Use the recursive helper to build the entire section tree
  // This supports both the Biography (text paragraphs) and Contact (nested form tags)
  const wrapperElement = createElementFromJSON(sectionData);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderPageContent(data, pageName) {
    ...
    // CSS hook for specific page styling
    ...
    // Route Content
    if (data.cardGrid) {
        renderCardGrid(targetContainer, data.cardGrid);
    } else if (data.contentSection) {
        // Now handles both Biography and Contact Form structures
        renderContentSection(targetContainer, data.contentSection);
  ...
  async function loadPage(pageName, addToHistory = true) {
    ...
    // Note: 'contactForm' type is no longer needed as contact.json is now a 'contentSection'
